...
import { editServiceGroup, addServiceGroup, addServicesData, deleteServiceGroup, deleteServicesData, editServiceData, updateServiceData } from 'src/utils/services';
...
const addHandler: Handler = async (req, res) => {
  const data = JSON.parse(req.body) as Service & { groupName: string } | undefined;
...
const editHandler: Handler = async (req, res) => {
  const data = JSON.parse(req.body) as Service & { oldName: string; groupName: string } | undefined;
  ...
  try {
    ...
    res.status(200).json({ msg: `编辑 ${data.groupName}/${data.oldName} 成功` });
  } catch {
    res.status(500).json(generatorRespError('服务器错误'));
  }
};
