...
export const serviceNameUpperAtom = atomWithStorage<boolean>(
  "home-page-service-name-upper",
  true
);
...
export default function GroupCard(props: ServiceGroup) {
  ...
  const move = (to: "up" | "down") => {
    ...
    const next =
      to === "up"
        ...
        : Math.min(current + 1, servicesData.length);
    const neoData = arrayMove(servicesData, current, next);
    handleUpdateServices(neoData);
  };

  const SortableBox: any = useMemo(
    () =>
      SortableContainer((props: any) => {
        return (
          <section className="mt-0 p-2 w-full rd-2">{props.children}</section>
        );
      }),
    []
  );
  ...
  const SortableCard = useMemo(
    () =>
      SortableElement((props: React.ComponentProps<typeof ServiceCard>) => {
  ...
  return (
    <div className=" groupgroup">
      <Collapse
          ...
          (
            <div className="flex flex-row justify-between align-center">
              <div className="flex justify-start items-start">
                ...
                <p className="position-relative ml-2 link text-4 ">
              ...
              <div className="flex flex-1  text-4.5 justify-end items-center mr-8 ">
                ...
                <Spacer />
                <div
                  onClick={() => handleDeleteServiceGroup(name)}
                  className={`i-carbon-trash-can transition-all ${
                    isEdit ? "visible op-100" : "invisible op-0"
                  } cursor-pointer z999`}
                />
                <Spacer />
                <div
                  onClick={() => move("up")}
                  className={`i-carbon-arrow-up transition-all ${
                    isEdit ? "visible op-100" : "invisible op-0"
                  } cursor-pointer z999`}
                />
                <Spacer />
                <div
                  onClick={() => move("down")}
                  className={`i-carbon-arrow-down transition-all ${
                    isEdit ? "visible op-100" : "invisible op-0"
                  } cursor-pointer z999`}
                />
              </div>
        ...
        className="p-0 m-0"
      >
        <SortableBox distance={1} axis="xy">
          {services.length !== 0 ? (
